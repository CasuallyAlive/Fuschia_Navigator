...
void motor_init(struct MotorStruct *n_motorL, struct MotorStruct *n_motorR, uint32_t psc, uint32_t arr) {
	...
	pwm_init(n_motorL, n_motorR);
	encoder_init(n_motorL, n_motorR, psc, arr);
	ADC_init();
}
...
void pwm_init(struct MotorStruct *motorL, struct MotorStruct *motorR) {		
